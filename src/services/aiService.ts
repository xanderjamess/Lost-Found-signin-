...
export async function matchItemWithAI(description: string, items: Item[]): Promise<{ itemId: string, confidence: number }[]> {
  if (!process.env.GEMINI_API_KEY) {
    console.warn("GEMINI_API_KEY not found. AI matching will be simulated.");
    return items.slice(0, 2).map(item => ({ itemId: item.id, confidence: 0.85 }));
  }
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `
        Given the following lost item description: "${description}"
        And the following list of found items: ${JSON.stringify(items.filter(i => i.status === 'found'))}
        
        Identify the top 3 most likely matches. Return a JSON array of objects with "itemId" and "confidence" (0-1).
...
export async function analyzeReportWithAI(report: Item, otherItems: Item[]): Promise<any> {
  if (!process.env.GEMINI_API_KEY) {
    ...
    return {
      ...
      potentialMatches: otherItems.slice(0, 2).map((item: any) => ({
        id: item.id,
        title: item.title,
        matchReason: "Similar category and location description. (Simulated)"
      })),
      recommendation: report.status === 'found' ? "Verify ownership by asking for specific details not mentioned in the report." : "Check the Student Union lost and found bin."
    };
  }
