    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full overflow-hidden flex flex-col md:flex-row h-full sm:h-auto sm:max-h-[90vh]"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Image Section */}
          <div className="md:w-1/2 relative h-48 sm:h-64 md:h-auto bg-gray-100 flex-shrink-0">
            <img
            ...
            <div className="absolute top-4 left-4 sm:top-6 sm:left-6">
              <span className={`px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-widest shadow-sm ${statusColors[item.status]}`}>
                {item.status}
              </span>
            </div>
            {item.status === 'claimed' && (
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center backdrop-blur-[1px]">
                <div className="bg-green-500 text-white px-6 py-2 sm:px-8 sm:py-3 rounded-full font-display font-bold text-sm sm:text-lg uppercase tracking-widest shadow-2xl transform -rotate-12 border-2 sm:border-4 border-white">
                  Claimed
                </div>
              </div>
            )}
          ...
          <div className="md:w-1/2 p-6 sm:p-8 md:p-12 overflow-y-auto">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-2xl sm:text-3xl font-display font-bold text-primary mb-2">{item.title}</h2>
                <div className="flex items-center text-[10px] sm:text-sm text-gray-400">
                  <span className="bg-gray-100 px-2 py-1 rounded text-[9px] sm:text-[10px] font-bold uppercase tracking-wider mr-3">
                    ID: {item.id.slice(-6)}
                  </span>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="flex items-start">
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Location</h4>
                    <p className="text-sm font-medium text-neutral-dark">{item.location}</p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Category</h4>
                    <p className="text-sm font-medium text-neutral-dark">{item.category}</p>
                  </div>
                </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Current Location</h4>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between mb-2">
                <div className="flex items-center">
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Item Reference ID</h4>
                    <p className="text-sm font-mono font-bold text-neutral-dark">{item.id.toUpperCase()}</p>
                  </div>
                </div>
                <button 
                  onClick={() => {
              ...
              {reporter && (
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center">
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Reported By</h4>
                    <p className="text-sm font-bold text-neutral-dark">{reporter.name}</p>
                    <p className="text-[10px] text-gray-500">{reporter.email}</p>
                  </div>
                </div>
              )}
                ...
                >
                  ...
                  <form onSubmit={handleClaimSubmit}>
                    <textarea
                      ...
                      className="w-full p-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/20 mb-4 text-sm"
                      rows={3}
                      value={claimMessage}
                    ...
                    <div className="mb-6 space-y-2">
                      <h5 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Photo Proof (Optional)</h5>
                      <div className={`relative w-full rounded-xl overflow-hidden border transition-all ${
                        uploadError ? 'border-red-300 bg-red-50/10' : 'border-gray-200'
                      }`}>
                        {proofImageUrl ? (
                          ...
                          <div className="relative w-full py-8 border-2 border-dashed border-transparent flex flex-col items-center justify-center text-gray-400 hover:text-primary transition-all group overflow-hidden">
                            <input 
                              type="file" 
                              accept="image/*" 
                        ...
                        {isUploading && (
                          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-xs flex flex-col items-center justify-center text-center p-3 z-20">
                            <div className="w-8 h-8 rounded-full border-3 border-white/20 border-t-primary animate-spin mb-2"></div>
                            <p className="text-white font-bold text-[10px] uppercase tracking-widest mb-1">Uploading proof...</p>
                            ...
                            <p className="text-white text-[9px] font-mono mt-0.5">{uploadProgress}%</p>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {isUploading ? (
                          <>
                            <span className="w-3.5 h-3.5 rounded-full border-2 border-white/25 border-t-white animate-spin"></span>
                            <span>Uploading...</span>
                          </>
                        ) : (
                      ...
                      <button 
                        ...
                        className="px-6 py-2 text-sm text-gray-500 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Cancel
                      </button>
                    </div>
              ...
              {!showClaimForm && (
                <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-4">
                  {item.status === 'found' && !isReporter ? (
                    ...
                    <button 
                      onClick={handleClaimClick}
                      className="flex-grow btn-accent py-4 flex items-center justify-center text-white"
                    >
