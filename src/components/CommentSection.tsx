  ...
  return (
    <div className="mt-8 pt-8 border-t border-slate-100">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-lg font-display font-bold text-slate-800">Community Discussion</h3>
      ...
      {currentUser ? (
        <form onSubmit={handleSubmit} className="mb-8">
          <div className="relative group">
            ...
            <button
              ...
              className="absolute bottom-3 right-3 p-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-all disabled:opacity-50 disabled:scale-95"
            >
              <Send size={18} />
            </button>
          </div>
        ...
        <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 text-center mb-8">
        ...
        <AnimatePresence mode="popLayout">
          {comments.length > 0 ? (
            comments.map((comment) => (
              ...
              >
                <div className="flex gap-4">
                  <div className="flex-shrink-0 w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 font-bold border-2 border-white shadow-sm">
                  ...
                  <div className="flex-grow">
                    <div className="flex items-baseline justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        {comment.isHidden && (
                          <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">HIDDEN</span>
                        )}
                      </div>
                    ...
                    <p className={`text-sm leading-relaxed mb-3 ${comment.isHidden ? 'text-slate-400 italic' : 'text-slate-600'}`}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {currentUser && !isAdmin && currentUser.id !== comment.userId && !comment.isFlagged && (
                        <button 
                          ...
                          className="flex items-center gap-1 text-[10px] font-bold text-slate-400 hover:text-amber-500 transition-colors"
                        >
                          <ShieldAlert size={12} />
                          FLAG
                        </button>
                          ...
                          <div className="relative">
                            ...
                            <div className="mt-2 flex justify-end gap-2">
                              ...
                              <button 
                                ...
                                className="bg-primary text-white text-[10px] font-bold px-3 py-1 rounded-lg hover:bg-primary/90 transition-all disabled:opacity-50"
                              >
                                SEND
                              </button>
                            </div>
                      ...
                      <div className="mt-4 pl-4 border-l-2 border-slate-50 space-y-4">
                        {comment.replies.map(reply => (
                          <div key={reply.id} className="flex gap-3">
                            <div className="flex-shrink-0 w-8 h-8 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 font-bold text-xs border border-slate-100">
                              {reply.userName.charAt(0)}
                            </div>
                            <div className="flex-grow">
                              <div className="flex items-baseline justify-between mb-0.5">
                                <span className="text-xs font-bold text-slate-800">{reply.userName}</span>
                                <span className="text-[9px] text-slate-400">{new Date(reply.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                              </div>
                              <p className="text-xs text-slate-600">{reply.content}</p>
                              {(isAdmin || currentUser?.id === reply.userId) && (
                                <button 
                                  onClick={() => onDeleteComment(reply.id)}
                                  className="text-[9px] font-bold text-slate-400 hover:text-red-500 mt-1"
                                >
                                  DELETE
                                </button>
                              )}
            ...
            <div className="text-center py-12 px-4 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
              ...
              <h4 className="text-sm font-bold text-slate-500 mb-1">No comments yet</h4>
              <p className="text-xs text-slate-400">Be the first to share helpful information about this item.</p>
            </div>
          )}
        </AnimatePresence>
      </div>
