...
export default function LoginForm({ initialRole, onSuccess, onSignup, onBack }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If it looks like a student ID (not an email), find the email from firestore
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (userDoc && !userDoc.exists()) {
        try {
          // If first time login, create the profile
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
        <motion.div 
          ...
          className="max-w-md w-full glass-card p-8 md:p-10 bg-white/90 backdrop-blur-xl shadow-2xl"
        >
          <div className="text-center mb-10">
            <h1 className="text-3xl font-display font-bold text-slate-900 mb-2">Forgot Password</h1>
            <p className="text-slate-500 font-medium">Enter your school email to receive a reset link</p>
          </div>
          ...
          {resetSent ? (
            <div className="text-center py-6">
              ...
              <p className="text-slate-500 mb-8">Please check your inbox for instructions to reset your password.</p>
              <button 
            ...
            <form onSubmit={handleResetPassword} className="space-y-6">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-widest">School Email</label>
              ...
              <button 
                type="button"
                onClick={() => setIsResetMode(false)}
                className="w-full text-slate-400 hover:text-primary transition-colors text-[10px] font-bold uppercase tracking-widest mt-4"
              >
                Cancel
              </button>
            </form>
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      ...
      >
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-display font-bold text-slate-900 mb-2 tracking-tight transition-all">{roleTitle}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center">
            <input 
              id="remember-me" 
              type="checkbox" 
              className="h-4 w-4 text-primary border-slate-200 rounded focus:ring-primary/20" 
            />
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className={`w-full py-4 rounded-xl text-white font-bold flex items-center justify-center transition-all shadow-lg ${buttonBg} disabled:opacity-70 uppercase tracking-widest text-sm`}
          >
            {isLoading ? (
              <>
        ...
        <div className="mt-8 text-center space-y-4">
          ...
          <button 
            onClick={onBack}
            className="text-slate-400 hover:text-primary transition-colors text-[10px] font-bold uppercase tracking-widest block w-full"
          >
            ← Back to selection
          </button>
        </div>
